...
int set_f0_and_power( char *ffile )
{
	...
	org_tf = totalframe;
	totalframe = prosBuf.nFrame;

	if( totalframe > org_tf )  {
		f0.data = (double *) realloc( f0.data, (totalframe+1)*sizeof(double) );
		if( f0.data == NULL )  {
			ErrMsg("Memory allocation error !  (in read_pros_file)\n");
			restart(1);
		}
		power.data = (double *) realloc( power.data, (totalframe+1)*sizeof(double) );
		if( power.data == NULL )  {
			ErrMsg("Memory allocation error !  (in read_pros_file)\n");
			restart(1);
		}
	...
	for( i=0; i<=totalframe; ++i )  {
		f0.data[i] = prosBuf.fr_f0[i];
		power.data[i] = prosBuf.fr_power[i];
	}
/*
	printf( "totalframe: %d\n", totalframe );
	for( i=0; i<=10; ++i )  {
