...
var TinyMCE_PastePlugin = {
	...
	_insertWordContent : function(content) { 
		if (content && content.length > 0) {
			...
			// convert <br> to paragraphs when from Ichitaro or NotePad
			if (content.match(/<FONT face=/)) {
				if (content.match(/<BR>/)) {
					content = content.replace(/^<FONT.*<\/FONT>$/, '<P>$&</P>');
	...
	//		content = content.replace(/<p>&nbsp;<\/p>/gi, '');
			...
			// remove rt tags
			content = content.replace(/<rt>.*?<\/rt>/gi, '');

			// preserve empty tr/td/th tags
			content = content.replace(/(<t([rdh])[^>]*>)(<\/t\2>)/gi, '$1&nbsp;$3');
